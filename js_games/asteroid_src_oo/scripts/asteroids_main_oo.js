...
class Main  extends Game.Main {
   constructor()
   {
      ...
      this.player = new Player(new Vector(GameHandler.width / 2, GameHandler.height / 2), new Vector(0.0, 0.0), 0.0);
      ...
      updateActorPosition(actor)
      {
         ...
         console.log("updateActorPosition: Actor type " + looperHigh++ + " "+ actor.constructor.name);
 ...
 */
class AttractorScene extends Game.Scene  {
   constructor(game)
   {
      //add aab
      super(false, null);
      ...
      onInitScene()
      {
         ...
         for (var i=0; i<8; i++)
         {
            ...
            this.actors.push(new Asteroid(pos, vec, randomInt(3,4)));
         }
      ...
      sceneRendererWelcome(ctx)
      {
         ctx.fillStyle = ctx.strokeStyle = "white";
         var t = (BITMAPS ? Game.centerFillText : Game.centerDrawText);
         t(ctx, "Press SPACE or click to start", "18pt Courier New", GameHandler.height*0.5);
         t = (BITMAPS ? Game.fillText : Game.drawText);
         t(ctx, "by Kevin Roast", "10pt Courier New", 16, 624);
      }
      
      sceneRendererInfo(ctx)
      {
         ctx.fillStyle = ctx.strokeStyle = "white";
         var t = (BITMAPS ? Game.fillText : Game.drawText);
         t(ctx, "How to play...", "14pt Courier New", 40, 320);
         t(ctx, "Arrow keys to rotate, thrust, shield. SPACE to fire.", "14pt Courier New", 40, 350);
         t(ctx, "Pickup the glowing power-ups to enhance your ship.", "14pt Courier New", 40, 370);
         t(ctx, "Watch out for enemy saucers!", "14pt Courier New", 40, 390);
         t(ctx, "Press S to enable or disable sound.", "14pt Courier New", 40, 410);
         t(ctx, "Press R to switch between Modern and Retro graphics.", "14pt Courier New", 40, 430);
      }
      
      sceneRendererScores(ctx)
      {
         ctx.fillStyle = ctx.strokeStyle = "white";
         var t = (BITMAPS ? Game.centerFillText : Game.centerDrawText);
 ...
 */
class GameOverScene extends Game.Scene {
   constructor(game)
   {
      super(false, interval);
      this.game = game;
...
/**
 * GameCompleted scene class.
 * 
 * @namespace Asteroids
 * @class GameCompleted
 */
class GameCompleted extends Game.Scene {
   constructor(game)
   {
     //add aab
     super(false, interval);
...
class GameScene extends Game.Scene {
   constructor(game, wave)
   {
       ...
       */
      resetPlayerActor(persistPowerUps)
      {
         this.actors.push([this.player]);
        
         // reset the player position
            this.player.position.x = GameHandler.width / 2;
            this.player.position.y = GameHandler.height / 2;
            this.player.vector.x = 0.0;
            this.player.vector.y = 0.0;
            this.player.heading = 0.0;
            this.player.reset(persistPowerUps);
...
*/
      ...
      onBeforeRenderScene()
      {
         ...
         {
            this.enemies.push(new EnemyShip(this, (this.wave < 3 ? 0 : randomInt(0, 1))));
            this.enemyShipCount++;
            this.enemyShipAdded = GameHandler.frameStart;
         }
      ...
      onKeyDownHandler(keyCode)
      {
         switch (keyCode)
         {
            ...
            case KEY.E:
            {
               if (DEBUG)
               {
                  this.enemies.push(new EnemyShip(this, randomInt(0, 1)));
                  return true;
               }
               break;
            }
      ...
      updateActors()
      {
         for (var i = 0, j = this.actors.length; i < j; i++)
         {
            ...
            for (var n = 0; n < actorList.length; n++)
            {
               ...
               {
         		console.log("updateActors: Actor type " + actor.constructor.name);
                  this.game.updateActorPosition(actor);
               }
            }
         }
               ...
               if (bulletPos.distance(enemy.position) <= bulletRadius + enemy.radius())
               {
                  ...
                  if (effectRad === 0)
                  {
                     ...
                     else
                     {
                        ...
                        var effect = new PlayerBulletImpact(bullet.position, bullet.vector);
            ...
            for (n = 0, m = this.enemies.length, z; n < m; n++)
            {
               ...
               if (enemy instanceof Asteroid)
               {
                  if (bulletPos.distance(enemy.position) <= bulletRadius + enemy.radius())
                  {
                     ...
                     {
                        // add a bullet impact particle effect to show the hit
                        var effect = new EnemyBulletImpact(bullet.position, bullet.vector);
                        this.effects.push(effect);
                     }
       ...
       */
      generatePowerUp(enemy, force)
      {
         if (this.collectables.length < 5 &&
             (force || randomInt(0, ((enemy instanceof Asteroid) ? 25 : 1)) === 0))
         {
            ...
            this.collectables.push(new PowerUp(
               new Vector(enemy.position.x, enemy.position.y - (enemy.size * 8)),
               vec));
         }
      }
       ...
       */
      destroyEnemy(enemy, parentVector, player)
      {
         if (enemy instanceof Asteroid)
         {
         ...
         else if (enemy instanceof EnemyShip)
         {
            ...
            var boom = new EnemyExplosion(enemy.position.clone(), enemy.vector.clone(), enemy);
            this.effects.push(boom);
       ...
       *
       * @param asteroid {Asteroid} The parent asteroid that has been destroyed
       * @param parentVector {Vector} Vector of the impacting object e.g. a bullet
       */
      generateBabyAsteroids(asteroid, parentVector)
      {
