...
class Shape {
    ...
    getCellArray() {
        var s = this;
        return mapcar(function(coord) {
                return [coord[0] + s._pos[0], coord[1] + s._pos[1]];
            },
...
class Grid {
    ...
    constructor(cols, rows, color, bordercolor, x, y, width, height, node) {
    ...
    eliminateFullRows() {
        ...
        for (var r = 0; r < this._rows; ++r) {
            if (rowFull(r)) {
                ...
                m.show()
                global_b.push(m);
                var btimeout = setTimeout("Message.prototype.hide_bonus()", 3000)
                var bo = true;
                eliminateRow(r);
                ++lines;
            }
...
class Message {
    ...
    constructor(txt, position, style) {
    ...
    show() {
        suspendRedraw();
        var svgR = document.getElementById("svgroot");
        var svgBoard = svgR.getElementById("board");
...
function canPlace(shape, grid) {
    ...
    return every(function(coord) {
            return grid.cellInBounds(coord) &&
...
function show(shape, grid) {
    ...
    mapc(function(coord) {
            grid.colorCell(coord, shape.getCellColor());
        },
        shape.getCellArray());
    unsuspendRedraw();
}
